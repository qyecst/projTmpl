...
class JsonFormatter:
    ...
    def format(record):
        ...
        # 转换成localtime再转换成新的时间格式(XXXX-XX-XX XX:XX:XX)
        obj['created'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(obj['created']))
...
file_name = os.path.join(BASE_DIR, 'site.log')
...
# logger设置
my_logger = logging.getLogger('site')
